...
public class GMLWriter {
	...
	private void writeEdges(final Writer writer, final List<EdgeData> edges) throws IOException {
		for (EdgeData e : edges) {
			Edge edge = e.getData();
			writeEdgeProperties(writer, Integer.parseInt(edge.getSource()), 
					Integer.parseInt(edge.getTarget()), edge.getWeight());
		}
	}

	private void writeEdgeProperties(final Writer writer,
			final Integer source, final Integer target, final Double weight) throws IOException {
		writer.write(TAB);
		...
		writeKey(writer, GMLTokens.VALUE);
		writeNumberProperty(writer, weight);
		writer.write(TAB);
		writer.write(CLOSE_LIST);
	}
